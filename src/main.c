...
void update_objects() {
    Player_update(&config->players[0], config->window_width, config->window_height);
    ...
    if(config->players[0].isShooting)
    {
        Bullet_spam(CBox_center(&config->players[0].box),
                    config->players[0].box.box.angle,
                    config->bullets, &config->players[0].weapon);
    }

    Bullet_update(config->bullets, config->window_width, config->window_height);

    Explosion_update(config->explosions, config->window_width, config->window_height);
}

void Background_render(V2 topLeft, double w, double h, float r, float g, float b)
{
...
void render_objects() {
    Background_render((V2) {0.0, 0.0}, config->window_width, config->window_height, 0.2, 0.2, 0.2);
