...
static void Enemy_update_pos(Enemy *enemies, int index) {
  ...
  enemy->collision_box.box.tl = V2add(enemy->collision_box.box.tl, enemy->vel);
}

int Enemy_count() { return config->enemiesCount; }
...
void Enemy_spawn(V2 topLeft, Enemy *enemies) {
  ...
  enemy.collision_box.box = Quad_from(topLeft, enemySize, enemySize);
...
void Enemy_update(Enemy *enemies, Player *players, int activePlayers) {
  ...
  for (i = 0; i < config->enemiesCount; i++) {
    ...
    for (j = 0; j < activePlayers; j++) {
      double tmpDistance = distanceBetween(
          CBox_center(&enemies[i].collision_box), CBox_center(&players[j].box));
