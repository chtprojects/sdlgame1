...
void Quad_update_position(Quad *q, V2 newTopLeft) { q->tl = newTopLeft; }
...
void Quad_render(const Quad *q) {
  ...
  float trx =
      centerX + ((q->tl.x + q->h - centerX) * c - (q->tl.y - centerY) * s);
  float try
    = centerY + ((q->tl.x + q->h - centerX) * s + (q->tl.y - centerY) * c);

  float blx =
      centerX + ((q->tl.x - centerX) * c - (q->tl.y + q->w - centerY) * s);
  float bly =
      centerY + ((q->tl.x - centerX) * s + (q->tl.y + q->w - centerY) * c);

  float brx = centerX +
              ((q->tl.x + q->h - centerX) * c - (q->tl.y + q->w - centerY) * s);
  float bry = centerY +
