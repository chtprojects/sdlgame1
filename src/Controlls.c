  ...
  for(i=0;i<c->activePlayers;i++)
  {
      ...
      {  
		...
		float treshold = width/4;
                ...
                {
//                  float angle = get_angle_between_points(inWorldX,inWorldY,analogLeftX,analogLeftY);
...
//                   float angle = get_angle_between_points(inWorldX,inWorldY, analogRightX,analogRightY);
...
void Controlls_handle_events(Config* c,SDL_Event event)
{
	...
	case SDL_CONTROLLERDEVICEADDED:
        ...
        case SDL_KEYDOWN:
            ...
            case SDL_KEYUP:
                 switch( event.key.keysym.sym ) {
                    ...
                    case SDLK_w:
                    upKeyPressed=0;
                         Player_move(&c->players[0], (V2) {c->players[0].vel.x / c->players[0].speed.x, 0});
                    ...
                    case SDLK_a:
                    leftKeyPressed=0;
                         Player_move(&c->players[0], (V2) {0, c->players[0].vel.y / c->players[0].speed.y});
		...
		handle_finger_press(0,c);
            ...
            handle_mouse_motion((V2) { event.motion.x, event.motion.y, }, c);
		break;
