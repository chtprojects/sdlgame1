...
static float ORTHO_MATRIX[16]= {0.0012f,  0,  0, 0,
                                  0,  -7.8,  0, -1,
                                      0,  0, -1, 0,
                                      0,  0,  0, 1};
...
static const GLbyte vShaderStr[] =
...
{
	 __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "laod shader ..", glGetError());
	 ...
	 if(shader == 0)
	 {
	         __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "shader ==0 return...", glGetError());

	 return 0;
	 }
	         __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "shader created", glGetError());
	 ...
	 __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "shader complilation failed retuning..", glGetError());

	 return 0;
	 }
	 __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "shader compiled", glGetError());

	 return shader;
}

static void setOrtho(float left, float right, float bottom, float top, float near, float far)
{
...
void setup_opengl( int width, int height )
{
        ...
        __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "init setup open", glGetError());
	// Create the program object
	programObject = glCreateProgram();
	__android_log_print(ANDROID_LOG_VERBOSE, "SDL", "program created", glGetError());

	if(programObject == 0){
	   	__android_log_print(ANDROID_LOG_VERBOSE, "SDL", "program ==0 return", glGetError());

	   return;
	   }
	vertexShader = LoadShader(GL_VERTEX_SHADER, vShaderStr);
 	fragmentShader = LoadShader(GL_FRAGMENT_SHADER, fShaderStr);
		__android_log_print(ANDROID_LOG_VERBOSE, "SDL", "shaders laoded", glGetError());
	...
	glLinkProgram(programObject);
		__android_log_print(ANDROID_LOG_VERBOSE, "SDL", "program linking...", glGetError());
	...
	if(!linked)
	{
	   ...
	   if(infoLen > 1)
	   {
	      ...
	      __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "eror linking program", glGetError());
	      free(infoLog);
	   }

	   glDeleteProgram(programObject);

	   __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "program not linked... deleting...", glGetError());
 ...
 // Set the viewport
 	 __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "program linked", glGetError());
