...
static void Particle_remove(Explosion *explosions,  int explosionIndex, int particleIndex)
{	
   if(config->explosionsCount<=0 || explosionIndex>=config->explosionsCount)
	return; 
   
   if(explosions[explosionIndex].particlesCount<=0 || particleIndex>=explosions[explosionIndex].particlesCount)
	return; 
   
   Particle tmp = explosions[explosionIndex].particles[explosions[explosionIndex].particlesCount-1];
	...
	for(i=0;i<config->explosionsCount;i++)	
	{
		...
		for(j=0;j<explosions[i].particlesCount;j++)
		{
			...
			if(explosions[i].type==SPIRAL){
				...
				Particle_move(p,2*PI*percent); //todo need better time base movement
			}
			else{
