...
static void on_check(Bullet *bullets, Enemy *enemies, int bulletPos,
                     int enemyPos, Explosion *explosions) {
...
static void shield_hit(Enemy *enemies, int enemyPos, int shieldPos,
...
static col_order BULLET_ENEMY_COL(int a, int b) {
  if (collisions_arr[a]->owner == BULLET_OWNER &&
      collisions_arr[b]->owner == ENEMY_OWNER)
    return OK;
  if (collisions_arr[a]->owner == ENEMY_OWNER &&
      collisions_arr[b]->owner == BULLET_OWNER)
    return REV;
...
static col_order PLAYER_ENEMY_COL(int a, int b) {
  if (collisions_arr[a]->owner == PLAYER_OWNER &&
      collisions_arr[b]->owner == ENEMY_OWNER)
    return OK;
  if (collisions_arr[a]->owner == ENEMY_OWNER &&
      collisions_arr[b]->owner == PLAYER_OWNER)
    return REV;
...
static col_order ENEMY_SHIELD_COL(int a, int b) {
  if (collisions_arr[a]->owner == ENEMY_OWNER &&
      collisions_arr[b]->owner == SHIELD_OWNER)
    return OK;
  if (collisions_arr[a]->owner == SHIELD_OWNER &&
      collisions_arr[b]->owner == ENEMY_OWNER)
    return REV;
    ...
    for (j = i + 1; j < ca_len; j++) {
      if (Collision_quads(&collisions_arr[i]->box, &collisions_arr[j]->box)) {
        ...
        if ((order = BULLET_ENEMY_COL(i, j)) != -1) {
          num_be_cols++;
          order == OK
              ? on_check(config->bullets, config->enemies,
                         collisions_arr[i]->ownerIdx,
                         collisions_arr[j]->ownerIdx, config->explosions)
              : on_check(config->bullets, config->enemies,
                         collisions_arr[j]->ownerIdx,
                         collisions_arr[i]->ownerIdx, config->explosions);
          ...
          order == OK
              ? shield_hit(config->enemies, collisions_arr[i]->ownerIdx,
                           collisions_arr[j]->ownerIdx, config->explosions)
              : shield_hit(config->enemies, collisions_arr[j]->ownerIdx,
                           collisions_arr[i]->ownerIdx, config->explosions);
        }
        ncol++;
      }
